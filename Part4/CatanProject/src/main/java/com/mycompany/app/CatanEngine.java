...
public class CatanEngine implements IGameController {
	...
	/**
	 * List of observers to notify on state changes
	 */
	...
	public boolean requestBuildRoad(int playerID, int edgeID) {
		...
		if (player == null)
			return false;
		boolean result = buildingService.buildRoad(playerID, edgeID, player);
		if (result)
			notifyObservers();
		return result;
	}
	...
	public boolean requestBuildCity(int playerID, int nodeID) {
		...
		if (player == null)
			return false;
		boolean result = buildingService.buildCity(playerID, nodeID, player);
		if (result)
			notifyObservers();
		return result;
	}
	...
	/**
	 * Detach an observer so it no longer receives updates
	 * 
	...
	/**
	 * Notify all attached observers that the game state has changed
	 */
